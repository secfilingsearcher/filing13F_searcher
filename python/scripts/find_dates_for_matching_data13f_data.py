...
intersection = set_of_edgar_filing_accession_no.intersection(set_of_data13f_accession_no)

set_of_dates_with_matching_data = set()
for accession_no in intersection:
    dates_with_matching_data = EdgarFiling.query.with_entities(EdgarFiling.filing_date).group_by(
        EdgarFiling.filing_date).filter(EdgarFiling.accession_no == accession_no).all()
    for row in dates_with_matching_data:
        for date in row:
