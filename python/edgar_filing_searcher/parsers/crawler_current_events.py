...
def get_subdirectories_for_specific_date(full_date: date):
    ...
    if full_date.year == 1994:
        search_url = base_url.join("/").join(f'{full_date.year}').join("/") \
            .join(f'QTR{quarter}').join("/").join(f'company.{subdirectory_date_94}.idx')
    elif 1995 <= full_date.year <= 1997:
        search_url = base_url.join("/").join(f'{full_date.year}').join("/") \
            .join(f'QTR{quarter}').join("/").join(f'company.{subdirectory_date_95_97}.idx')
    else:
        search_url = base_url.join("/").join(f'{full_date.year}').join("/") \
            .join(f'QTR{quarter}').join("/").join(f'company.{subdirectory_date_after_1998}.idx')
...
def ensure_13f_filing_detail_urls(cik_ascension_subdirectories):
    ...
    specific_date_13f_filing_detail_urls = \
        ["https://www.sec.gov/Archives/edgar/data/".join(subdirectory).join(
